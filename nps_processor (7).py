...
class NPSProcessor:
    ...
    def getcode(self):
        ...
        if not self.provider_ranking_nps.empty:
            last_provider_id = self.provider_ranking_nps['provider_id'].max()
            print(f"Last provider_id in existing data: {last_provider_id}")
        ...
        nps_query = "SELECT * FROM DATA_SCIENCE_DB.PUBLIC.NPS_PROVIDER_RAW"
        if last_encounter_date:
            try:
                last_encounter_date = datetime.strptime(last_encounter_date, '%Y-%m-%d').strftime('%Y-%m-%d')
                nps_query += f" WHERE Encounter_date > '{last_encounter_date}'"
            except ValueError as e:
                print(f"Date conversion error: {e}")
                return

        new_nps_data = getcode(nps_query)
        ...
        provider_ranking_query = "SELECT * FROM DATA_SCIENCE_DB.PUBLIC.PROVIDER_RANKING_FINAL"
        if last_provider_id:
            provider_ranking_query += f" WHERE provider_id > {last_provider_id}"

        new_provider_ranking_nps = getcode(provider_ranking_query)
        if not new_provider_ranking_nps.empty:
            print(f"New records loaded from PROVIDER_RANKING_FINAL: {len(new_provider_ranking_nps)}")
            self.provider_ranking_nps = pd.concat([self.provider_ranking_nps, new_provider_ranking_nps], ignore_index=True)
